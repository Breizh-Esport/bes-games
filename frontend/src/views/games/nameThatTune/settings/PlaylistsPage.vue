    ...
    <div>
        <div class="row row-space">
            <div>
                ...
                <p class="muted">
                    Create playlists and add YouTube tracks (title + URL).
                </p>
            </div>
        ...
        <template v-else>
            ...
            <div class="row">
                <div class="col">
                    <label class="label" for="newPlaylistName"
                        >New playlist name</label
                    >
                    <input
                        id="newPlaylistName"
                ...
                <div class="actions">
                    <button
                        ...
                        :disabled="creating || !newPlaylistNameTrimmed"
                    >
                    ...
                    <div class="playlist-body">
                        <div v-if="renamingId === pl.id" class="row">
                            ...
                            <div class="actions">
                                <button
                                    class="btn"
                                    @click="applyRename(pl)"
                                    :disabled="renamingBusy || !renameTrimmed"
                                >
                            ...
                            >
                                <div class="track-thumb">
                                    <img
                                        ...
                                        alt="thumbnail"
                                    />
                                    <div v-else class="track-thumb-fallback">
                                        note
                                    </div>
                                </div>
                                <div class="track-main">
                                    <template v-if="editingItemId === it.id">
                                        ...
                                        <div class="actions">
                                            <button
                                                class="btn"
                                                @click="saveTrackTitle(pl, it)"
                                                :disabled="
                                                    editingItemBusy ||
                                                    !editItemTitleTrimmed
                                                "
                                            >
                                                {{
                                                    editingItemBusy
                                            ...
                                            <button
                                                ...
                                                :disabled="editingItemBusy"
                                            >
                                                Cancel
                                            </button>
                                        </div>
                                        <p
                                            v-if="editItemErrorId === it.id"
                                            class="error"
                                        >
                                            {{ editItemError }}
                                        </p>
                                ...
                                >
                                    <button
                                        class="btn btn-ghost"
                                        @click="beginEditTrack(pl, it)"
                                    >
                                        Edit title
                                    </button>
                                    <button
                                        class="btn btn-danger"
                                        @click="deleteTrack(pl, it)"
                        ...
                        <div class="row">
                            <div class="col">
                                ...
                                />
                                <div class="muted small">
                                    Title and thumbnail are fetched
                                    automatically.
                                </div>
                            </div>
                            <div class="actions">
                        ...
                        <p v-if="addItemErrorId === pl.id" class="error">
                            {{ addItemError }}
                        </p>
                    </div>
                </details>
...
const editingItemBusy = ref(false);
const editItemTitle = ref("");
const editItemErrorId = ref("");
const editItemError = ref("");
const deletingItemId = ref("");

const newPlaylistNameTrimmed = computed(() =>
    (newPlaylistName.value || "").trim(),
);
const renameTrimmed = computed(() => (renameName.value || "").trim());
const editItemTitleTrimmed = computed(() => (editItemTitle.value || "").trim());
...
async function createPlaylist() {
    ...
    error.value = "";
    try {
        await api.createPlaylist(gameId, {
            name: newPlaylistNameTrimmed.value,
        });
...
async function addItem(pl) {
    ...
    try {
        ...
        await api.addPlaylistItem(gameId, pl.id, { youtubeUrl });
        addTrackUrl[pl.id] = "";
        await loadPlaylists();
    } catch (e) {
...
async function saveTrackTitle(pl, it) {
    if (!editItemTitleTrimmed.value) return;
    editingItemBusy.value = true;
    editItemErrorId.value = "";
    editItemError.value = "";
    try {
        await api.patchPlaylistItem(gameId, pl.id, it.id, {
            title: editItemTitleTrimmed.value,
        });
        editingItemId.value = "";
        editItemTitle.value = "";
        await loadPlaylists();
    } catch (e) {
        editItemErrorId.value = it.id;
        editItemError.value = e?.message || "Failed to update track title";
    } finally {
        editingItemBusy.value = false;
    }
}

async function deleteTrack(pl, it) {
    const ok = window.confirm(
        `Delete "${it.title}" from "${pl.name}"? This cannot be undone.`,
    );
    if (!ok) return;
    deletingItemId.value = it.id;
    editItemErrorId.value = "";
    editItemError.value = "";
    try {
        await api.deletePlaylistItem(gameId, pl.id, it.id);
        await loadPlaylists();
    } catch (e) {
        editItemErrorId.value = it.id;
        editItemError.value = e?.message || "Failed to delete track";
    } finally {

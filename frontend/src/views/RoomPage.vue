<template>
  <main class="page">
    <header class="header">
      <div class="brand">
        ...
        <p class="subtitle muted" v-if="snapshot">
          {{ snapshot.name }} • {{ isOwner ? 'Owner view' : 'Player view' }}
        </p>
      ...
      <div class="right">
        <div class="badge" :class="auth.isAuthenticated.value ? 'badge-ok' : 'badge-warn'">
          ...
          <span class="badge-value">{{ auth.isAuthenticated.value ? 'Authenticated' : 'Anonymous' }}</span>
        </div>
    ...
    <section class="card" v-if="error">
      ...
      <div class="row">
        <button class="btn" @click="reloadAll" :disabled="loadingAny">Retry</button>
      </div>
    </section>
    ...
    <section class="card" v-else>
      <div class="row row-space">
        <div>
          ...
          <p class="muted">Live roster (name, picture, score). Owner can kick and change score.</p>
        </div>

        <div class="actions">
          <button class="btn btn-ghost" @click="reloadAll" :disabled="loadingAny">
            {{ loadingAny ? 'Refreshing…' : 'Refresh' }}
          </button>
          <button class="btn btn-ghost" @click="copyRoomId" :disabled="!roomId">Copy room id</button>
        </div>
      </div>
      ...
      <div v-else class="roster">
        <div v-if="!snapshot.players?.length" class="muted">No players yet.</div>

        <div v-for="p in snapshot.players" :key="p.playerId" class="player">
          <div class="avatar">
            <img v-if="p.pictureUrl" :src="p.pictureUrl" alt="avatar" />
            <div v-else class="avatar-fallback">{{ initialsOf(p.nickname) }}</div>
          </div>

          <div class="player-main">
            <div class="player-name">
              <span class="name">{{ p.nickname }}</span>
              <span class="pill" :class="p.connected ? 'pill-ok' : 'pill-dim'">{{
                p.connected ? 'online' : 'offline'
              }}</span>
              <span v-if="p.playerId === playerId" class="pill pill-me">you</span>
              <span v-if="p.sub && p.sub === snapshot.ownerSub" class="pill pill-owner">owner</span>
            </div>
          ...
          <div class="player-actions" v-if="isOwner">
            <button class="btn btn-ghost" @click="scoreDelta(p.playerId, -1)" :disabled="busyOwner">-1</button>
            <button class="btn btn-ghost" @click="scoreDelta(p.playerId, +1)" :disabled="busyOwner">+1</button>
            <button class="btn btn-ghost" @click="scoreSetPrompt(p.playerId, p.score)" :disabled="busyOwner">
              Set…
            </button>
            <button class="btn btn-danger" @click="kick(p.playerId)" :disabled="busyOwner">Kick</button>
          </div>
        </div>
      </div>
    </section>
    ...
    <section class="card">
      <div class="row row-space">
        <div>
          ...
          <p class="muted">
            Owner loads a playlist and controls track/seek/play. Players can control volume locally.
          </p>
        </div>
      </div>
      ...
      <template v-else>
        <div class="row">
          <div class="col">
            ...
            <div class="strong">
              ...
              <span v-if="snapshot.playlist" class="muted small">({{ snapshot.playlist.items?.length || 0 }} tracks)</span>
            </div>
          </div>
          ...
          <div class="col">
            ...
            <div class="strong">
              <span v-if="snapshot.playback?.track">{{ snapshot.playback.track.title }}</span>
          ...
          <div class="col">
            ...
            <div class="strong">
              ...
              <span class="muted small">• pos {{ Math.floor((snapshot.playback?.positionMs || 0) / 1000) }}s</span>
            </div>
          </div>
        </div>
        ...
        <div class="grid">
          <div class="card-inner" v-if="isOwner">
            ...
            <div class="row">
              <div class="col">
                <label class="label" for="playlistSelect">Load playlist</label>
                <select id="playlistSelect" class="input" v-model="selectedPlaylistId">
                  <option value="">Select…</option>
                  <option v-for="pl in myPlaylists" :key="pl.id" :value="pl.id">
                    {{ pl.name }} ({{ pl.items?.length || 0 }})
                  </option>
                </select>
              </div>
              <div class="actions">
                <button class="btn" @click="loadSelectedPlaylist" :disabled="busyOwner || !selectedPlaylistId">
                  Load
                </button>
                <button class="btn btn-ghost" @click="refreshMyPlaylists" :disabled="busyOwner">
                  Refresh playlists
                </button>
              </div>
            </div>
            ...
            <div class="row">
              <div class="actions">
                <button class="btn" @click="togglePause" :disabled="busyOwner || !canControlPlayback">
                  {{ snapshot.playback?.paused ? 'Play' : 'Pause' }}
                </button>
                <button class="btn btn-ghost" @click="prevTrack" :disabled="busyOwner || !canControlPlayback">
                  Prev
                </button>
                <button class="btn btn-ghost" @click="nextTrack" :disabled="busyOwner || !canControlPlayback">
                  Next
                </button>
                <button class="btn btn-ghost" @click="restartTrack" :disabled="busyOwner || !canControlPlayback">
                  Restart
                </button>
              </div>
            </div>
            ...
            <div class="row">
              <div class="col">
                <label class="label" for="seek">Seek (seconds)</label>
                <input
                  id="seek"
                  class="input"
                  type="number"
              ...
              <div class="actions">
                <button class="btn btn-ghost" @click="seekTo" :disabled="busyOwner || !canControlPlayback">Seek</button>
              </div>
            </div>

            <div class="hint muted small">
              Note: actual audio playback is not implemented yet (YouTube embed/player). These controls broadcast state to
          ...
          <div class="card-inner">
            ...
            <div class="row">
              <div class="col">
                ...
                <input id="volume" class="input" type="range" min="0" max="100" v-model.number="volume" />
              </div>
              <div class="col">
            ...
            <div class="row">
              ...
              <div class="actions">
                <button class="btn btn-buzz" @click="buzz" :disabled="!playerId || buzzing">
            ...
            <div v-if="lastBuzz" class="result">
              <div class="result-line">
                Last buzz:
                <strong>{{ lastBuzz.player?.nickname || lastBuzz.player?.Nickname || 'unknown' }}</strong>
                <span class="muted small">• {{ formatRelative(lastBuzz.ts) }}</span>
              </div>
            </div>

            <div class="hint muted small">
              Tip: Joining is currently done from Home. The room remembers your <code class="code">playerId</code> only
              in-memory on this page.
            </div>
          </div>
        </div>
      </template>
    ...
    <section class="card">
      ...
      <p class="muted">
        This page expects you already joined from Home. If you didn’t, you can join here quickly.
      </p>

      <div class="row">
        <div class="col">
          <label class="label" for="quickNick">Nickname (optional)</label>
          <input id="quickNick" v-model="quickNick" class="input" type="text" placeholder="Anonymous" />
        </div>
        <div class="col">
          <label class="label" for="quickPic">Picture URL (optional)</label>
          <input id="quickPic" v-model="quickPic" class="input" type="url" placeholder="https://…" />
        </div>
        <div class="actions">
          <button class="btn" @click="quickJoin" :disabled="joining || !roomId">
            {{ joining ? 'Joining…' : playerId ? 'Re-join (new playerId)' : 'Join' }}
          </button>
          <button class="btn btn-ghost" @click="leave" :disabled="leaving || !playerId">Leave</button>
        </div>
      </div>
...
const canControlPlayback = computed(() => !!snapshot.value?.playlist?.playlistId && (snapshot.value?.playlist?.items?.length || 0) > 0)
...
async function reloadAll() {
  ...
  error.value = ''
  try {
    snapshot.value = await api.getRoom(props.roomId)
...
async function loadSelectedPlaylist() {
  ...
  ownerError.value = ''
  try {
    await api.loadPlaylist(props.roomId, { playlistId: selectedPlaylistId.value })
...
async function togglePause() {
  ...
  try {
    ...
    await api.pause(props.roomId, { paused })
...
async function setTrackIndex(trackIndex, { paused = undefined, positionMs = undefined } = {}) {
  busyOwner.value = true
  ownerError.value = ''
  try {
    await api.setPlayback(props.roomId, { trackIndex, paused, positionMs })
    // Do not apply REST response; wait for WS `room.snapshot`.
  } catch (e) {
...
async function seekTo() {
  ...
  ownerError.value = ''
  try {
    await api.seek(props.roomId, { positionMs: Math.floor(sec * 1000) })
    // Do not apply REST response; wait for WS `room.snapshot`.
  } catch (e) {
...
async function kick(pid) {
  ...
  ownerError.value = ''
  try {
    await api.kick(props.roomId, { playerId: pid })
...
async function scoreDelta(pid, delta) {
  ...
  ownerError.value = ''
  try {
    await api.addScore(props.roomId, { playerId: pid, delta })
...
async function scoreSetPrompt(pid, current) {
  ...
  ownerError.value = ''
  try {
    await api.setScore(props.roomId, { playerId: pid, score: Math.trunc(n) })
    // Do not apply REST response; score updates come via WS `room.snapshot`.
  } catch (e) {
...
async function quickJoin() {
  ...
  playerError.value = ''
  try {
    const res = await api.joinRoom(props.roomId, {
...
async function leave() {
  ...
  playerError.value = ''
  try {
    await api.leaveRoom(props.roomId, { playerId: playerId.value })
    playerId.value = ''
...
async function buzz() {
  ...
  playerError.value = ''
  try {
    await api.buzz(props.roomId, { playerId: playerId.value })
  } catch (e) {
...
function connectWS() {
  ...
  const url = roomWebSocketUrl(props.roomId)
  ...
  ws.onmessage = (evt) => {
    try {
      ...
      if (msg?.type === 'buzzer') {
        lastBuzz.value = { ts: msg.ts || new Date().toISOString(), player: msg.payload?.player }
        return
      }

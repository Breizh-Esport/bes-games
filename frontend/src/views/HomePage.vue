<template>
  <main class="page">
    ...
    <section class="grid">
      <article v-for="g in games" :key="g.id" class="card game-card">
        ...
        <div class="game-actions">
          <RouterLink class="btn" :to="gameLink(g.id)">Open</RouterLink>
        </div>
      </article>
    ...
    <section class="card">
      ...
      <p class="muted">
        Authentication is currently a placeholder. Set an arbitrary <code class="code">sub</code> to simulate being
        logged in (sent as <code class="code">X-User-Sub</code> to the backend).
      </p>
...
onMounted(() => {
  ;(async () => {
    ...
    try {
      ...
      gamesError.value = e?.message || 'Failed to load games (using fallback list)'
    } finally {
